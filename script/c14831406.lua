...
function c14831406.fcheck(tp,sg,fc)
	return sg:FilterCount(Card.IsLocation,nil,LOCATION_DECK)<=1
end
function c14831406.gcheck(sg)
	return sg:FilterCount(Card.IsLocation,nil,LOCATION_DECK)<=1
end
	...
	if chk==0 then
		...
		local mg4=Duel.GetMatchingGroup(Card.IsCanBeFusionMaterial,tp,LOCATION_DECK,0,nil)
			if mg4:GetCount()>0 then
				mg2:Merge(mg4)
				Auxiliary.FCheckAdditional=c14831406.fcheck
				Auxiliary.GCheckAdditional=c14831406.gcheck
			end
...
function c14831406.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local mg2=Duel.GetMatchingGroup(Card.IsCanBeFusionMaterial,tp,LOCATION_HAND+LOCATION_ONFIELD,0,e:GetHandler()):Filter(c14831406.filter,nil,e)
	local mg4=Duel.GetMatchingGroup(Card.IsCanBeFusionMaterial,tp,LOCATION_DECK,0,nil)
		if mg4:GetCount()>0 then
			mg2:Merge(mg4)
		end
