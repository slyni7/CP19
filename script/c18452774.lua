...
function cm.nfil1(c,tp,xc)
	return ((c:IsFaceup() and c:IsLoc("M")) or
		(Duel.GetFlagEffect(tp,m)<1 and c:IsCode(12800777,73001017) and c:IsLoc("H")))
...
function cm.tfil3(c)
	return c:IsType(TYPE_MONSTER) and not c:IsSummonableCard() and c:IsAbleToGrave()
end
function cm.tar3(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if oc:GetOriginalAttribute()&ATTRIBUTE_WIND>0 then
		e:SetCategory(CATEGORY_TOGRAVE+CATEGORY_SEARCH)
	else
		e:SetCategory(CATEGORY_TOGRAVE)
	end
end
function cm.ofil31(c)
	return c:IsCode(18452786) and c:IsSSetable()
end
function cm.ofil32(c)
	return c:IsCode(12800777,73001017) and c:IsAbleToHand()
end
function cm.op3(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local sg=Duel.SMCard(tp,cm.tfil3,tp,"D",0,1,1,nil)
	if #sg>0 then
	...
	if oc:GetOriginalAttribute()&ATTRIBUTE_WIND>0 then
		...
		local tg=Duel.SMCard(tp,cm.ofil31,tp,"D",0,1,1,nil)
		if #tg>0 and Duel.SSet(tp,tg) then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
			local g=Duel.SMCard(tp,cm.ofil32,tp,LSTN("D"),0,0,1,nil)
			if #g>0 then
				Duel.BreakEffect()
