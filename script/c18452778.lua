...
function cm.nfil1(c,tp,xc)
	return ((c:IsFaceup() and c:IsLoc("M")) or
		(Duel.GetFlagEffect(tp,m)<1 and c:IsCode(47606319,76305638) and c:IsLoc("H")))
...
function cm.tar3(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return Duel.IEMCard(Card.IsType,tp,"O","O",1,nil,TYPE_SPELL+TYPE_TRAP)
	end
	local dg=Duel.GMGroup(Card.IsType,tp,"O","O",nil,TYPE_SPELL+TYPE_TRAP)
	Duel.SOI(0,CATEGORY_DESTROY,dg,#dg,0,0)
	local oc=e:GetLabelObject()
	if oc:GetOriginalAttribute()&ATTRIBUTE_EARTH>0 then
		e:SetCategory(CATEGORY_DESTROY+CATEGORY_SEARCH)
	else
		e:SetCategory(CATEGORY_DESTROY)
	end
end
function cm.ofil3(c)
	return c:IsCode(47606319,76305638) and c:IsAbleToHand()
end
...
function cm.tfil5(c)
	return c:IsAttribute(ATTRIBUTE_EARTH) and c:IsAbleToHand() and c:IsFaceup()
end
