...
function cm.tfil21(c,e,tp)
	return c:IsSetCard(0x2d6) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and not c:IsCode(m)
		and not Duel.IEMCard(cm.tfil22,tp,"O",0,1,nil,c:GetCode())
end
...
function cm.tar2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return Duel.IEMCard(cm.tfil21,tp,"HDG",0,1,nil,e,tp) and Duel.GetLocCount(tp,"M")>0
	end
...
function cm.op2(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SMCard(tp,cm.tfil21,tp,"HDG",0,1,1,nil,e,tp)
	if #g>0 then
