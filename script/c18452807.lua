...
function cm.tfil2(c,tp)
	return c:IsSetCard(0x2d6) and c:IsAbleToGrave() and not c:IsCode(m)
		and (c:IsLoc("D") or Duel.IsPlayerCanDraw(tp,1))
end
function cm.tar2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return Duel.IEMCard(cm.tfil2,tp,"HDO",0,1,nil,tp) and Duel.GetFieldGroupCount(tp,LSTN("D"),0)>0
	end
...
function cm.op2(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SMCard(tp,cm.tfil2,tp,"HDO",0,1,1,nil,tp)
	local tc=g:GetFirst()
	if tc then
		local loc=tc:GetLocation()
		Duel.SendtoGrave(g,REASON_EFFECT)
		if loc&LSTN("D")<1 then
