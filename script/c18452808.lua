...
function cm.initial_effect(c)
	...
	aux.AddCodeList(c,18452805,18452806)
...
function cm.pfun1(g)
	...
	local sg=g:Clone()
	sg:AddCard(ec)
	return sg:IsExists(Card.IsModuleCode,1,nil,18452805) and sg:IsExists(Card.IsModuleCode,1,nil,18452806)
end
...
function cm.tfil2(c,tp)
	return (c:IsControler(tp) and Duel.GetLocCount(tp,"M",tp,LOCATION_REASON_CONTROL)>0) or (c:IsControler(1-tp) and c:IsControlerCanBeChanged())
end
function cm.tar2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then
		return chkc:IsLoc("M") and cm.tfil2(chkc,tp)
	end
	if chk==0 then
		return Duel.IETarget(cm.tfil2,tp,"M","M",1,nil,tp)
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONTROL)
	local g=Duel.STarget(tp,cm.tfil2,tp,"M","M",1,1,nil,tp)
...
function cm.op2(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) then
		if tc:IsControler(1-tp) then
			...
			if Duel.GetLocCount(tp,"M",tp,LOCATION_REASON_CONTROL)>0 then
...
function cm.tfil3(c)
	...
	return c:IsSetCard(0x2d6) and c:IsFaceup() and not aux.IsCodeListed(c,18452805) and not aux.IsCodeListed(c,18452806) and ct2<1
end
...
function cm.val10(e,c)
	return (c:IsSetCard(0x2d6) and not aux.IsCodeListed(c,18452805) and not aux.IsCodeListed(c,18452806)) or e:GetHandler():GetEquipTarget()==c
end