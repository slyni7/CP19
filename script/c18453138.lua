...
function cm.tfil11(c)
	return c:IsSetCard(0x2e3) and c:IsCanOverlay() and not c:IsCode(m)
end
...
function cm.tar1(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then
		return chkc:IsControler(tp) and chkc:IsLoc("M") and cm.tfil12(chkc)
	end
	if chk==0 then
		return Duel.IEMCard(cm.tfil11,tp,"D",0,1,nil) and Duel.IETarget(cm.tfil12,tp,"M",0,1,nil)
	end
	Duel.STarget(tp,cm.tfil12,tp,"M",0,1,1,nil)
end
