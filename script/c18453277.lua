...
function cm.initial_effect(c)
	c:EnableReviveLimit()
	aux.AddXyzProcedure(c,aux.FBF(Card.IsSetCard,0x2eb),1,2)
...
function cm.tfil1(c)
	return c:IsSetCard(0x2eb) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
...
function cm.tfun2(g,e,tp)
	return g:GetSum(Card.GetLevel)<=2 and g:IsExists(Card.IsCanBeSpecialSummoned,2,nil,e,0,tp,false,false,POS_FACEUP_DEFENSE)
end
function cm.tar2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then
		return g:CheckSubGroup(cm.tfun2,2,2,e,tp) and Duel.GetLocCount(tp,"M")>1
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local tg=g:SelectSubGroup(tp,cm.tfun2,false,2,2,e,tp)
