...
function cm.ofil2(c)
	return c:IsRace(RACE_FAIRY+RACE_FIEND) and c:IsLevelAbove(5) and c:IsAbleToHand() and c:IsSummonableCard()
		and not (c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsRace(RACE_FAIRY))
end
...
function cm.op4(e,tp,eg,ep,ev,re,r,rp)
	...
	e1:SetTR("M",0)
	e1:SetTarget(aux.TargetBoolFunction(Card.IsRace,RACE_FAIRY+RACE_FIEND))
	e1:SetValue(cm.oval41)
	e1:SetReset(RESET_PHASE+PHASE_END)
	Duel.RegisterEffect(e1,tp)
end
function cm.oval41(e,te)
	return e:GetOwnerPlayer()~=te:GetOwnerPlayer()
end
function cm.ofil5(c)
	return c:IsFaceup() and c:IsSetCard(0x2ef) and c:IsAttackable()
end
function cm.op5(e,tp,eg,ep,ev,re,r,rp)
	...
	local res=Duel.IEMCard(cm.ofil5,tp,"M",0,1,nil) and Duel.IEMCard(aux.TRUE,tp,0,"M",1,nil)
	if res and Duel.SelectEffectYesNo(tp,c,aux.Stringid(m,3)) then
		local g=Duel.SMCard(tp,cm.ofil5,tp,"M",0,1,1,nil)
		local ac=g:GetFirst()
		local bg=Duel.SMCard(tp,aux.TRUE,tp,0,"M",1,1,nil)
		local bc=bg:GetFirst()
		local e1=MakeEff(c,"FC")
		e1:SetCode(EVENT_BATTLE_DESTROYING)
		e1:SetReset(RESET_PHASE+PHASE_DAMAGE+RESET_EVENT+RESETS_STANDARD)
		e1:SetLabelObject(ac)
		e1:SetOperation(cm.oop51)
		Duel.RegisterEffect(e1,tp)
		Duel.CalculateDamage(ac,bc)
	end
end
function cm.oop51(e,tp,eg,ep,ev,re,r,rp)
	local ac=e:GetLabelObject()
	if eg:IsContains(ac) then
