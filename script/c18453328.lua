...
function cm.ofil2(c)
	return c:IsRace(RACE_FAIRY+RACE_FIEND) and (c:IsFaceup() or c:IsLoc("G"))
end
...
function cm.ofil4(c)
	return c:IsSetCard(0x2ef) and c:IsType(TYPE_MONSTER) and not c:IsCode(m)
end
function cm.op4(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and ft>0 and Duel.SelectEffectYesNo(tp,c,aux.Stringid(m,2)) then
...
function cm.con5(e,c,minc)
	if c==nil then
		return e:GetHandler():IsSummonType(SUMMON_TYPE_ADVANCE)
	end
	local tp=c:GetControler()
	local g=Duel.GMGroup(cm.nfil5,tp,"G",0,nil)
	local mi,ma=c:GetTributeRequirement()
	if mi<minc then
		mi=minc
	end
	...
	return #g>=mi and Duel.GetLocCount(tp,"M")>0
end
...
function cm.op5(e,tp,eg,ep,ev,re,r,rp,c)
	...
	local sg=g:Select(tp,mi,ma,nil)
	Duel.SendtoHand(sg,nil,REASON_EFFECT)
	Duel.ConfirmCards(1-tp,sg)
end