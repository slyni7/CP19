...
function cm.ofil2(c)
	return c:IsLevelAbove(5) and c:IsAbleToHand() and c:IsSummonableCard()
		and ((c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsRace(RACE_FAIRY)) or (c:IsAttribute(ATTRIBUTE_DARK) and c:IsRace(RACE_FIEND)))
end
...
function cm.op5(e,tp,eg,ep,ev,re,r,rp)
	...
	local res=c:GetFlagEffect(m)==0 and re:IsActiveType(TYPE_MONSTER+TYPE_SPELL)
	if res and Duel.SelectEffectYesNo(tp,c,aux.Stringid(m,3)) then
			c:RegisterFlagEffect(m,RESET_EVENT+0x1ec0000+RESET_PHASE+PHASE_END)
			local g=Group.CreateGroup()
			Duel.ChangeTargetCard(ev,g)
			Duel.ChangeChainOperation(ev,cm.oop51)
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
			local dg=Duel.SMCard(tp,aux.TRUE,tp,0,"O",0,1,nil)
			if #dg>0 then
				Duel.BreakEffect()
