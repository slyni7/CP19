...
s.listed_names={18453502}
function s.sprfilter(c,tp)
	return c:IsSetCard(0x22f2) and c:IsAbleToRemoveAsCost() and aux.SpElimFilter(c,true)
end
...
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(aux.FilterFaceupFunction(Card.IsSetCard,0x22f2),tp,LOCATION_ONFIELD,0,1,nil)
		and Duel.IsExistingTarget(nil,tp,0,LOCATION_ONFIELD,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g1=Duel.SelectTarget(tp,aux.FilterFaceupFunction(Card.IsSetCard,0x22f2),tp,LOCATION_ONFIELD,0,1,1,nil)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
...
function s.spfilter(c,e,tp)
	return c:IsCode(18453502) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
