...
function s.thfilter(c)
	return c:IsSetCard(0x12f2) and c:IsAbleToHand()
end
...
function s.costfilter(c)
	return c:IsSetCard(0x12f2) and c:IsDiscardable()
end
...
function s.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.IsExistingMatchingCard(s.costfilter,tp,LOCATION_HAND,0,1,nil) and Duel.GetFlagEffect(tp,id)==0 end
