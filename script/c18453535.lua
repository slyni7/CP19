...
function s.initial_effect(c)
	...
	e3:SetCategory(CATEGORY_SUMMON+CATEGORY_DESTROY)
	e3:SetCL(1,{id,1})
	WriteEff(e3,3,"CTO")
...
function s.pfun1(g,tp,xyz)
	if g:IsExists(s.pffil1,1,nil) then
		local g1=g:Filter(Card.IsXyzLevel,nil,xyz,7)
		local g2=g-g1
		if #g2>1 then
			return false
		end
	...
	return g:FilterCount(Card.IsLoc,nil,"M")==#g and g:FilterCount(Card.IsXyzLevel,nil,xyz,7)==#g
end
s.g1=nil
...
function s.tar3(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		return Duel.IEMCard(s.tfil3,tp,"HM",0,1,nil) and #dg>0
	end
