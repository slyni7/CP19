...
function s.initial_effect(c)
	c:EnableReviveLimit()
	Fusion.AddProcMixN(c,true,true,18453577,1,aux.FilterBoolFunctionEx(Card.IsRace,RACE_PYRO),2)
...
function s.tdcfilter(c,tp)
	return c:IsRace(RACE_PYRO) and c:IsReason(REASON_EFFECT) and c:IsControler(tp)
end
