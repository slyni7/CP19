...
function s.initial_effect(c)
	...
	Fusion.AddProcMix(c,true,true,18453582,aux.FilterBoolFunctionEx(Card.IsSetCard,0x12c4))
	-- Battle protection
	...
	e1:SetTarget(function(e,c) return c~=e:GetHandler() and c:IsRace(RACE_PYRO) end)
	e1:SetValue(1)
	c:RegisterEffect(e1)
...
s.listed_names={18453582}
function s.negcon(e,tp,eg,ep,ev,re,r,rp)
	return rp==1-tp and re:IsHasCategory(CATEGORY_SPECIAL_SUMMON) and Duel.IsChainNegatable(ev)
end
function s.tgfilter(c)
	return c:IsSetCard(0x12c4) and (c:IsFaceup() or c:IsLocation(LOCATION_HAND)) and c:IsAbleToGrave()
end
