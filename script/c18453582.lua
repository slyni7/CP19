...
function s.initial_effect(c)
	c:EnableReviveLimit()
	Fusion.AddProcMix(c,true,true,aux.FilterBoolFunctionEx(Card.IsSetCard,0x12c4),aux.FilterBoolFunctionEx(Card.IsRace,RACE_PYRO))
...
function s.thfilter(c)
	return c:IsSetCard(0x12c4) and c:IsAbleToHand()
end
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.thfilter,tp,LOCATION_DECK,0,1,nil)
		or Duel.IsPlayerCanDiscardDeck(tp,5) end
end
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,s.thfilter,tp,LOCATION_DECK,0,0,1,nil)
	if #g>0 then
...
function s.spfilter(c,e,tp)
	return c:IsSetCard(0x12c4) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.tgfilter(c)
	return c:IsSetCard(0x12c4) and c:IsAbleToGrave()
end
