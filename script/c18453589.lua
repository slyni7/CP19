...
function s.negcon(e,tp,eg,ep,ev,re,r,rp)
	return (re:IsActiveType(TYPE_MONSTER) or re:IsHasType(EFFECT_TYPE_ACTIVATE)) and Duel.IsChainNegatable(ev)
		and (Duel.IsExistingMatchingCard(aux.FilterFaceupFunction(Card.IsCode,46659709),tp,LOCATION_ONFIELD,0,1,nil)
		or Duel.IsExistingMatchingCard(aux.FilterFaceupFunction(Card.IsSetCard,0x12c4),tp,LOCATION_MZONE,0,1,nil))
end
...
function s.thfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x12c4) and c:IsMonster() and c:IsAbleToHand()
end
