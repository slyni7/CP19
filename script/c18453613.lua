...
function s.initial_effect(c)
	...
	e2:SetTarget(function(_,c) return c:IsSetCard(0x12c4) or c:IsType(TYPE_FUSION) end)
	e2:SetValue(500)
	c:RegisterEffect(e2)
...
s.listed_series={0x12c4}
function s.cfilter(c,tp)
	return c:IsRace(RACE_PYRO) and c:IsSetCard(0x12c4) and c:IsControler(tp) and c:IsReason(REASON_EFFECT)
end
function s.grcon(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(s.cfilter,1,nil,tp)
end
function s.grvfilter(c)
	return c:IsRace(RACE_PYRO) and c:IsLevelBelow(4) and c:IsAbleToGrave()
end
...
function s.grop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.SendtoGrave(g,REASON_EFFECT)>0 then
		...
		if tc:IsLocation(LOCATION_GRAVE) and not tc:IsSetCard(0x12c4) then
