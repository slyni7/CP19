...
function s.cost1(e,tp,eg,ep,ev,re,r,rp,chk)
	local rg=Duel.GMGroup(aux.SimorghBanishFilter1,tp,"OG",0,nil,ATTRIBUTE_WATER)
	if chk==0 then
		return #rg>0 and aux.SelectUnselectGroup(rg,e,tp,1,2,aux.SimorghBanishResult(ATTRIBUTE_WATER),0)
	end
	local g=Group.CreateGroup()
	while not aux.SimorghBanishResult(ATTRIBUTE_WATER)(g,e,tp,Group.CreateGroup()) do
		g=aux.SelectUnselectGroup(rg,e,tp,1,2,aux.SimorghBanishResult(ATTRIBUTE_WATER),1,tp,HINTMSG_REMOVE,nil,nil,false)
	end
	if #g==1 then
		local tc=g:GetFirst()
...
function s.cost2(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local rg=Duel.GMGroup(aux.SimorghBanishFilterAngel1,tp,"OG","M",nil,ATTRIBUTE_WATER,tp)
	if chk==0 then
		return ae and #rg>0 and aux.SelectUnselectGroup(rg,e,tp,1,2,aux.SimorghBanishResult(ATTRIBUTE_WATER),0)
	end
...
function s.cost3(e,tp,eg,ep,ev,re,r,rp,chk)
	local rg=Duel.GMGroup(aux.SimorghBanishFilter2,tp,"OH",0,nil,ATTRIBUTE_WATER)
	if chk==0 then
		return #rg>0 and aux.SelectUnselectGroup(rg,e,tp,1,2,aux.SimorghBanishResult(ATTRIBUTE_WATER),0)
	end
	local g=Group.CreateGroup()
...
function s.cost4(e,tp,eg,ep,ev,re,r,rp,chk)
	local ae=Duel.IsPlayerAffectedByEffect(tp,EFFECT_ANGEL_SIMORGH)
	local rg=Duel.GMGroup(aux.SimorghBanishFilterAngel2,tp,"OH","M",nil,ATTRIBUTE_WATER,tp)
	if chk==0 then
		return ae and #rg>0 and aux.SelectUnselectGroup(rg,e,tp,1,2,aux.SimorghBanishResult(ATTRIBUTE_WATER),0)
	end
	local ac=ae:GetHandler()
