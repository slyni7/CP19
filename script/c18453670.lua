...
s.square_mana={ATTRIBUTE_DARK}
s.custom_type=CUSTOMTYPE_SQUARE
function s.cfil1(c)
	return c:IsSetCard("마방") and c:IsAbleToGraveAsCost() and c:GetLevel()>0 and not c:IsCode(id)
end
function s.cost1(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return Duel.IEMCard(s.cfil1,tp,"DE",0,1,nil)
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SMCard(tp,s.cfil1,tp,"DE",0,1,1,nil)
...
function s.tar2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local c=e:GetHandler()
	if chkc then
		return chkc:IsLoc("G") and ckhc:IsControler(tp) and s.tfil21(chkc,e,tp,c)
	end
	if chk==0 then
		return Duel.IETarget(s.tfil21,tp,"G",0,1,nil,e,tp,c) and Duel.GetLocCount(tp,"M")>0
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.STarget(tp,s.tfil21,tp,"G",0,1,1,nil,e,tp,c)
...
function s.op2(e,tp,eg,ep,ev,re,r,rp)
	...
	if not tc:IsRelateToEffect(e) or not s.tfil21(tc,e,tp,c) then
		return
	end
