...
s.square_mana={ATTRIBUTE_WATER,ATTRIBUTE_WATER,ATTRIBUTE_WATER,ATTRIBUTE_WATER}
...
function s.cost1(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return Duel.IEMCard(s.cfil1,tp,"G",0,1,nil)
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SMCard(tp,s.cfil1,tp,"G",0,1,1,nil)
...
function s.tar2(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then
		return Duel.IEMCard(s.tfil21,tp,"DE",0,1,nil,e,tp,c)
	end
	Duel.SOI(0,CATEGORY_TOGRAVE,nil,1,tp,"DE")
end
...
function s.op2(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SMCard(tp,s.tfil21,tp,"DE",0,1,1,nil,e,tp,c)
	local tc=g:GetFirst()
	if not tc then
		return
	end
