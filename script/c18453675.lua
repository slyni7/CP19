...
s.square_mana={ATTRIBUTE_LIGHT,ATTRIBUTE_LIGHT,ATTRIBUTE_LIGHT,ATTRIBUTE_LIGHT,ATTRIBUTE_LIGHT,ATTRIBUTE_LIGHT}
s.custom_type=CUSTOMTYPE_SQUARE
function s.cfil1(c)
	return c:IsAbleToHandAsCost() and c:GetLevel()>0 and c:IsFaceup()
end
function s.cost1(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then
		return Duel.IEMCard(s.cfil1,tp,"M",0,1,c)
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RTOHAND)
	local g=Duel.SMCard(tp,s.cfil1,tp,"M",0,1,1,c)
...
function s.tar2(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then
		return Duel.IEMCard(s.tfil21,tp,"E",0,1,nil,e,tp,c) and Duel.GetLocCount(tp,"M")>0
	end
...
function s.op2(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SMCard(tp,s.tfil21,tp,"E",0,1,1,nil,e,tp,c)
	local tc=g:GetFirst()
	if not tc then
		return
	end
