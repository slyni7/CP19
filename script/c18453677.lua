...
function cm.pfil1(c)
	return c:IsSynchroType(TYPE_TUNER) or c:IsSetCard("原号")
end
function cm.pfun1(g)
...
function cm.tfil1(c)
	return c:IsSetCard("原号") and c:IsAbleToHand() and c:IsType(TYPE_SPELL+TYPE_TRAP)
end
...
function cm.tfil2(c,e,tp)
	return c:IsSetCard("原号") and not c:IsCode(m) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
