...
function s.ofil22(c,tp)
	if Duel.IEMCard(s.ofil21,tp,"O",0,1,nil) then
		if c:IsRace(RACE_ZOMBIE) and c:IsAbleToHand() then
			return true
		end
	end
	return c:IsCode(18453827) and c:IsAbleToHand()
end
...
function s.tar6(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return Duel.IEMCard(s.ofil22,tp,"D",0,1,nil,tp)
	end
...
function s.op6(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SMCard(tp,s.ofil22,tp,"D",0,1,1,nil,tp)
	if #g>0 then
		Duel.SendtoHand(g,nil,REASON_EFFECT)
		Duel.ConfirmCards(1-tp,g)
	end
