...
function s.initial_effect(c)
	...
	e3:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_CANNOT_NEGATE+EFFECT_FLAG_CANNOT_INACTIVATE+EFFECT_FLAG_DELAY)
...
function s.nfil2(c,tp,sc)
	return c:IsSetCard("vingt et un") and c:IsType(TYPE_PENDULUM) and c:IsFaceup()
...
function s.con2(e,c)
	...
	return Duel.IEMCard(s.nfil2,tp,"O",0,1,nil,tp,c)
end
function s.op2(e,tp,eg,ep,ev,re,r,rp,c)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g=Duel.SMCard(tp,s.nfil2,tp,"O",0,1,1,nil,tp,c)
...
function s.tfil6(c)
	return c:IsAbleToHand() and c:IsCode(18453886)
end
