...
function s.tar1(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return (Duel.CheckLocation(tp,LSTN("P"),0) or Duel.CheckLocation(tp,LSTN("P"),1))
			and Duel.IEMCard(s.tfil1,tp,"H",0,1,nil) and Duel.IsPlayerCanDraw(tp,1)
	end
	Duel.SOI(0,CATEGORY_DRAW,nil,0,tp,1)
end
function s.op1(e,tp,eg,ep,ev,re,r,rp)
	if (Duel.CheckLocation(tp,LSTN("P"),0) or Duel.CheckLocation(tp,LSTN("P"),1)) then
...
function s.tfil3(c)
	return c:IsAbleToHand() and c:IsCode(18453894)
end
...
function s.tfil6(c)
	return c:IsAbleToHand() and c:IsCode(18453887)
end
