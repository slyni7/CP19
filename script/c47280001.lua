...
function cm.eqfilter(c)
	return c:IsFaceup() and c:IsSetCard(0xe25) and not c:IsAttribute(ATTRIBUTE_WATER)
end
...
function cm.effilter(c,e,tp)
	return c:IsSetCard(0xe25) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and not c:IsAttribute(ATTRIBUTE_WATER)
end

function cm.efftg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(cm.effilter,tp,LOCATION_HAND+LOCATION_DECK,0,1,nil,e,tp) end
...
function cm.effop(e,tp,eg,ep,ev,re,r,rp)
	...
	g=Duel.SelectMatchingCard(tp,cm.effilter,tp,LOCATION_HAND+LOCATION_DECK,0,1,1,nil,e,tp)
