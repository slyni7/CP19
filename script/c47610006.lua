...
function cm.effilter(c)
	return not c:IsCode(m) and c:IsAbleToDeck() and c:IsSetCard(0x2a2)
end

function cm.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return Duel.IsExistingMatchingCard(cm.effilter,tp,LOCATION_GRAVE,0,1,nil) and Duel.IsPlayerCanDraw(tp,1) end
...
function cm.operation(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local g=Duel.SelectMatchingCard(tp,cm.effilter,tp,LOCATION_GRAVE,0,1,1,nil)
