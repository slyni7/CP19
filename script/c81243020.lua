...
function cm.tfil0(c)
	return c:IsAbleToGrave() and c:IsSetCard(0xc86) and c:IsType(0x1) and c:IsAttribute(0x20)
end
function cm.tfil1(c)
...
--레벨
function cm.nfil1(c)
	return c:IsFaceup() and c:IsAttribute(0x20)
end
function cm.cn3(e)
...
function cm.cn4(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsReason(REASON_COST)	and re:IsActivated() and re:GetHandler():IsSetCard(0xc86)
end
function cm.tg4(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return Duel.IsExistingMatchingCard(Card.IsAbleToHand,tp,0,0x0c,1,nil)
	end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,0,0x0c)
end
function cm.op4(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RTOHAND)
	local g=Duel.SelectMatchingCard(tp,Card.IsAbleToHand,tp,0,0x0c,1,1,nil)
	if #g>0 then
