...
function c95481208.initial_effect(c)
	...
	e3:SetTarget(c95481208.tdtg)
	e3:SetOperation(c95481208.tdop)
	c:RegisterEffect(e3)
	--special summon
...
function c95481208.tgfilter(c)
	return c:IsSetCard(0xd47) and c:IsAbleToGrave()
end
function c95481208.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c:IsRelateToEffect(e) then return end
	if Duel.SpecialSummon(c,0,tp,1-tp,false,false,POS_FACEUP_DEFENSE)~=0 then
		local tg=Duel.GetMatchingGroup(c95481208.tgfilter,tp,0,LOCATION_DECK,nil)
