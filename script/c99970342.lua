...
function cm.initial_effect(c)
	...
	e00:SetValue(ATTRIBUTE_FIRE)
	c:RegisterEffect(e00)
	
	--샐비지 / 장착
	local e1=MakeEff(c,"STo")
	e1:SetCategory(CATEGORY_TOHAND+CATEGORY_LEAVE_GRAVE+CATEGORY_EQUIP)
...
function cm.thfilter(c,ec)
	return c:IsType(TYPE_UNION) and c:IsSetCard(0xe10) and (c:IsAbleToHand() or c:CheckEquipTarget(ec) and aux.CheckUnionEquip(c,ec))
end
function cm.tar1(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and cm.thfilter(chkc,e:GetHandler()) end
	if chk==0 then return Duel.IsExistingTarget(cm.thfilter,tp,LOCATION_GRAVE,0,1,nil,e:GetHandler()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectTarget(tp,cm.thfilter,tp,LOCATION_GRAVE,0,1,1,nil,e:GetHandler())
...
function cm.op1(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) then
		if tc:IsAbleToHand() and (not c:IsFaceup() or not c:IsRelateToEffect(e)
			or not tc:CheckEquipTarget(c)
			or not aux.CheckUnionEquip(tc,c)
			or Duel.SelectYesNo(tp,aux.Stringid(m,2))) then
